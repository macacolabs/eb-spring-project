...
import com.ohgiraffers.restapi.auth.repository.RefreshTokenRepository;
...
public class AuthService {
    ...
    public TokenDTO login(LoginRequest request) {
        UserDetails userDetails = customUserDetailsService.loadUserByUsername(request.getMemberId());
        ...
        if(!member.isEnabled()) {
            throw new DisabledException("휴면상태인 계정입니다.");
        }
    ...
    @Transactional
    public TokenDTO refreshToken(String providedRefreshToken) {
        ...
        String memberId = jwtTokenProvider.getMemberIdFromJWT(providedRefreshToken);
        ...
        if(storedToken.getExpiryDate().before(new Date())) {
            throw new BadCredentialsException("리프레시 토큰 유효시간이 만료되었습니다.");
        }

        MemberDTO member = (MemberDTO) customUserDetailsService.loadUserByUsername(memberId);
        return issueTokens(member);
    }
    ...
    public void logout(String refreshToken) {
        ...
        String memberId = jwtTokenProvider.getMemberIdFromJWT(refreshToken);
    ...
    private TokenDTO issueTokens(MemberDTO member) {
